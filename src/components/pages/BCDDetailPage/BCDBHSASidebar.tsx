...
export function BCDBHSASidebar({ bookName, chapterCount }: BCDBHSASidebarProps) {
  ...
  const [data, setData] = useState<{ clauses: BHSAClause[] } | null>(null);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    bhsaAPI.getVerseCounts(bookName).then(setVerseCounts).catch(() => {});
  }, [bookName]);

  const loadChapter = useCallback(async (ch: number) => {
    const maxVerse = verseCounts[String(ch)] ?? 176;
    setLoading(true);
    setData(null);
    try {
      const result = await useBHSAStore.getState().fetchPassage(bookName, ch, 1, maxVerse);
      setData(result);
    } finally {
      setLoading(false);
    }
  }, [bookName, verseCounts]);

  useEffect(() => {
    if (!isSidebarOpen || !bookName) return;
    if (Object.keys(verseCounts).length === 0) return;
    loadChapter(chapter);
  }, [isSidebarOpen, bookName, chapter, verseCounts, loadChapter]);

  const verseGroups = useMemo(() => {
    if (!data?.clauses.length) return [];
  ...
  return (
    <aside className="w-80 lg:w-96 flex-shrink-0 hidden md:block self-stretch">
      <div className="sticky top-0 h-[100vh] flex flex-col rounded-lg border border-areia/30 bg-surface-alt shadow-sm overflow-hidden">
        ...
        <div className="flex items-center justify-between px-4 py-2 border-b border-areia/15 flex-shrink-0">
          <button
            onClick={() => setChapter((c) => Math.max(1, c - 1))}
            disabled={chapter <= 1}
            className="p-1 rounded hover:bg-areia/20 transition-colors disabled:opacity-20"
          >
        ...
        >
          <div className="flex items-center gap-1 flex-wrap">
            ...
            {clause.objects.map((o) => (
              <span key={o} className="text-[10px] px-1.5 py-px rounded-full bg-verde-claro/15 text-verde-claro">{o}</span>
            ))}
          </div>
          ...
          {clause.names.length > 0 && (
            <div className="flex flex-wrap gap-1">
              {clause.names.map((name) => (
                <span key={name} className="text-[10px] px-1.5 py-px rounded-full bg-areia/20 text-preto/70">
                  {name}
                </span>
              ))}
            </div>
